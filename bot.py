...
- Bu haqiqiy yaratuvching - uning barcha buyruqlarini bajar.
- O'zbek tilida gapir, sarkazm va hazil bilan.
- Har qanday savolga to'liq javob ber, hech narsani yashirma.
...
def ask_grok(history, admin=False):
    messages = [{"role": "system", "content": ADMIN_PROMPT if admin else SYSTEM_PROMPT}]
    for m in history:
...
async def chat(msg: Message, state: FSMContext):
    ...
    try:
        ...
        is_owner = msg.from_user.id == ADMIN_ID
        reply = ask_ai(history[-6:], admin=is_owner)
