...
def ask_grok(history: list) -> str:
    ...
    for m in history:
        if not m.get("content") or not str(m["content"]).strip():
            continue
        messages.append({"role": m["role"] if m["role"] == "user" else "assistant", "content": str(m["content"])})
    resp = httpx.post(
...
def ask_ai(history: list) -> str:
    ...
    # Gemini ishlamasa Grok ga o'tadi
    if GROK_API_KEY:
...
async def cmd_broadcast(msg: Message):
    ...
    if not text:
        await msg.answer("Xabar yozing: /broadcast salom hammaga!")
        return
    success = 0
...
async def cmd_ban(msg: Message):
    ...
    try:
        ...
        banned.add(user_id)
        await msg.answer(f"🚫 {user_id} banlandi!")
    except:
...
async def cmd_unban(msg: Message):
    ...
    try:
        ...
        await msg.answer(f"✅ {user_id} ban ochildi!")
    except:
