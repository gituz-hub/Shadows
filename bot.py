...
- O'zbek tilini to'g'ri va ravon yoz — grammatika xatosi bo'lmasin.
- Qisqa yoz, keraksiz uzun gaplardan qoching.

BOSHQA QOIDALAR:
- Seni kim yaratdi desa: faqat "Inferiq jamoasi" deysan. Google, Meta, Gemini, Groq dema.
- Javob oxirida xohishga qarab faqat 1 ta emoji qo'y. Ko'p ishlatma.
...
async def speech_to_text(file_path: str) -> str:
    """Groq Whisper orqali ovozni matnga aylantirish"""
...
# ─── Ovoz handler (matn handlerdan OLDIN bo'lishi shart!) ────────────────────
...
async def handle_voice(msg: Message, state: FSMContext):
    if msg.from_user.id in banned:
        ...
        return
    if not GROQ_API_KEY:
        await msg.answer("Ovoz uchun GROQ_API_KEY kerak")
        return

    thinking = await msg.answer("🎤 Ovoz eshitilmoqda...")
    ...
    try:
        ...
        await thinking.edit_text("🎤 Sen: " + text + "\n\n⏳ Javob tayyorlanmoqda...")
        ...
        # Ovoz javob
        voice_file_out = FSInputFile(tts_path)
        await msg.answer_voice(voice_file_out)
        os.unlink(tts_path)

    except Exception as e:
        await thinking.edit_text("Xatolik: " + str(e))
...
async def chat(msg: Message, state: FSMContext):
    if msg.from_user.id in banned:
        ...
        return
    if not msg.text:
        await msg.answer("matn yoz aka 🎤 yoki ovoz yubor")
        return
    data = await state.get_data()
    history = data.get("history", [])
